...
""" '^' (caret) ---------------------------------------------------------- """
# 文字列の先頭にマッチします。
# re.MULTILINEフラグ使用時は、改行の直後にもマッチします。
...
""" '$' (dollar sign) ---------------------------------------------------- """
# 文字列の末尾にマッチします。
# re.MULTILINEフラグ使用時は、改行の直前にもマッチします。
...
# 5-18. '(?:...)'
""" '(?:...)' ------------------------------------------------------------ """
# ... の正規表現にマッチしますが、このグループの部分文字列はマッチした後で
# 使用することはできません。

# """
text = "Mr Smith and Mrs. Johnson met Ms Brown."
matches = re.findall(r"(?:Mr|Mrs|Ms)\.?\s[A-Z][a-z]+", text)
print("'(?:Mr|Mrs|Ms)\.?\s[A-Z][a-z]+'マッチ:", matches)

# 出力：
#  '(?:Mr|Mrs|Ms)\.?\s[A-Z][a-z]+'マッチ: ['Mr Smith', 'Mrs. Johnson', 'Ms Brown']
# """
...
# 5-27. '(?(id/name)yes-pattern|no-pattern)'
...
matches = re.findall(pattern, text)
print("'(?(id/name)'マッチ:", matches)

# 出力：
#     '(?(id/name)'マッチ: [('<', 'user@host.com'), ('', 'user@host.com')]
# """
